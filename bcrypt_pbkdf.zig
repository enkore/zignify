// This is a literal translation of the following file from OpenBSD:
// $OpenBSD: bcrypt_pbkdf.c,v 1.13 2015/01/12 03:20:04 tedu
//
...
const bcrypt_words = 8;
const bcrypt_hashsize = bcrypt_words * 4;
const bcrypt_ciphertext = @as([32]u8, "OxychromaticBlowfishSwatDynamite".*);

fn bcrypt_hash(sha2pass: []const u8, sha2salt: []const u8, out: []u8) void {
    var state = Blowfish{};
    var cdata: [bcrypt_words]u32 = undefined;
    ...
    var j: usize = 0;
    i = 0;
    while (i < bcrypt_words) : (i += 1) {
        cdata[i] = Blowfish.toWord(&bcrypt_ciphertext, &j);
    }
    i = 0;
    while (i < 64) : (i += 1) {
        state.encrypt(&cdata);
    ...
    i = 0;
    while (i < bcrypt_words) : (i += 1) {
        out[4 * i + 3] = @truncate(u8, cdata[i] >> 24);
        out[4 * i + 2] = @truncate(u8, cdata[i] >> 16);
        out[4 * i + 1] = @truncate(u8, cdata[i] >> 8);
        out[4 * i + 0] = @truncate(u8, cdata[i] & 0xff);
    }
...
/// Performs a bcrypt_pbkdf key derivation based on *passphrase*, *salt* and *rounds*,
/// writing the result into *key*.
pub fn bcrypt_pbkdf(passphrase: []const u8, salt: []const u8, key: []u8, rounds: u32) !void {
    var out: [bcrypt_hashsize]u8 = undefined;
    var tmpout: [bcrypt_hashsize]u8 = undefined;
    ...
    if (passphrase.len == 0 or salt.len == 0 or key.len == 0 or key.len > out.len * out.len)
        return error.BCryptError;

    const stride = (key.len + out.len - 1) / out.len;
    var amt = (key.len + stride - 1) / stride;

    // Collapse password
    SHA512.hash(passphrase, &sha2pass, .{});

    var count: u32 = 1;
    var key_to_gen = key.len;

    // generate key in chunks of out.len
    while (key_to_gen > 0) : (count += 1) {
        const countsalt = [4]u8{
            @truncate(u8, count >> 24),
            @truncate(u8, count >> 16),
            @truncate(u8, count >> 8),
            @truncate(u8, count & 0xff),
        };
        ...
        // output the key material non-linearly (deviates from pbkdf2)
...
const Blowfish = struct {
    ...
    fn feistelF(state: Blowfish, x: u32) u32 {
    ...
    fn halfRound(state: Blowfish, i: u32, j: u32, n: usize) u32 {
        return i ^ state.feistelF(j) ^ state.subkeys[n];
    }

    fn encipher(state: Blowfish, halves: *Halves) void {
    ...
    fn encrypt(state: Blowfish, data: []u32) void {
...
// These test vectors come from PyCA/bcrypt, which also contains the OpenBSD bcrypt_pbkdf implementatin
// and some pieces of the OpenBSD libc/crypto (blowfish).
test "PyCA/bcrypt test vectors" {
    const Vector = struct { rounds: u32, passphrase: []const u8, salt: []const u8, key: []const u8 };
    const vectors = &[_]Vector{
        .{
            .rounds = 4,
            .passphrase = "password",
            .salt = "salt",
            .key = "\x5b\xbf\x0c\xc2\x93\x58\x7f\x1c\x36\x35\x55\x5c\x27\x79\x65\x98" ++
        ...
        .{
            ...
            .passphrase = "password",
            .salt = "\x00",
            .key = "\xc1\x2b\x56\x62\x35\xee\xe0\x4c\x21\x25\x98\x97\x0a\x57\x9a\x67",
        },
        .{
            .rounds = 4,
            .passphrase = "\x00",
            .salt = "salt",
            .key = "\x60\x51\xbe\x18\xc2\xf4\xf8\x2c\xbf\x0e\xfe\xe5\x47\x1b\x4b\xb9",
        },
        .{
            // nul bytes in password and string
            .rounds = 4,
            .passphrase = "password\x00",
            .salt = "salt\x00",
            .key = "\x74\x10\xe4\x4c\xf4\xfa\x07\xbf\xaa\xc8\xa9\x28\xb1\x72\x7f\xac" ++
                "\x00\x13\x75\xe7\xbf\x73\x84\x37\x0f\x48\xef\xd1\x21\x74\x30\x50",
        },
        .{
            .rounds = 4,
            .passphrase = "pass\x00wor",
            .salt = "sa\x00l",
            .key = "\xc2\xbf\xfd\x9d\xb3\x8f\x65\x69\xef\xef\x43\x72\xf4\xde\x83\xc0",
        },
        .{
            .rounds = 4,
            .passphrase = "pass\x00word",
            .salt = "sa\x00lt",
            .key = "\x4b\xa4\xac\x39\x25\xc0\xe8\xd7\xf0\xcd\xb6\xbb\x16\x84\xa5\x6f",
        },
        .{
            // bigger key
            .rounds = 8,
            .passphrase = "password",
            .salt = "salt",
            .key = "\xe1\x36\x7e\xc5\x15\x1a\x33\xfa\xac\x4c\xc1\xc1\x44\xcd\x23\xfa" ++
        ...
        .{
            ...
            .passphrase = "password",
            .salt = "salt",
            .key = "\x83\x3c\xf0\xdc\xf5\x6d\xb6\x56\x08\xe8\xf0\xdc\x0c\xe8\x82\xbd",
        },
        .{
            // longer password
            .rounds = 8,
            .passphrase = "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do " ++
            ...
            .salt = "salis\x00",
            .key = "\x10\x97\x8b\x07\x25\x3d\xf5\x7f\x71\xa1\x62\xeb\x0e\x8a\xd3\x0a",
        },
        .{
            // "unicode"
            .rounds = 8,
            .passphrase = "\x0d\xb3\xac\x94\xb3\xee\x53\x28\x4f\x4a\x22\x89\x3b\x3c\x24\xae",
            .salt = "\x3a\x62\xf0\xf0\xdb\xce\xf8\x23\xcf\xcc\x85\x48\x56\xea\x10\x28",
            .key = "\x20\x44\x38\x17\x5e\xee\x7c\xe1\x36\xc9\x1b\x49\xa6\x79\x23\xff",
        },
        .{
            // very large key
            .rounds = 8,
            .passphrase = "\x0d\xb3\xac\x94\xb3\xee\x53\x28\x4f\x4a\x22\x89\x3b\x3c\x24\xae",
            .salt = "\x3a\x62\xf0\xf0\xdb\xce\xf8\x23\xcf\xcc\x85\x48\x56\xea\x10\x28",
            .key = "\x20\x54\xb9\xff\xf3\x4e\x37\x21\x44\x03\x34\x74\x68\x28\xe9\xed" ++
        ...
        .{
            ...
            .rounds = 8,
            .passphrase = "\xe1\xbd\x88\xce\xb4\xcf\x85\xcf\x83\xcf\x83\xce\xb5\xcf\x8d\xcf" ++
                "\x82",
            .salt = "\xce\xa4\xce\xb7\xce\xbb\xce\xad\xce\xbc\xce\xb1\xcf\x87\xce\xbf" ++
                "\xcf\x82",
            .key = "\x43\x66\x6c\x9b\x09\xef\x33\xed\x8c\x27\xe8\xe8\xf3\xe2\xd8\xe6",
        },
    };
